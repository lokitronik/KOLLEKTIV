        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <div className="relative">
            <button 
              ...
              className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 border border-slate-700 text-xs font-medium text-slate-200 transition-colors touch-manipulation min-h-[36px]"
            ...
            {isLangOpen && (
              <>
                ...
                />
                <div className="absolute right-0 mt-1 w-28 py-1 bg-slate-800 border border-slate-700 rounded-lg shadow-xl transition-all z-50">
                  <button 
                    onClick={() => {
                      setLang('sv');
                      setIsLangOpen(false);
                    }} 
                    className={`w-full text-left px-3 py-2 text-xs hover:bg-slate-700 flex items-center justify-between touch-manipulation ${lang === 'sv' ? 'text-emerald-400 font-bold' : 'text-slate-300'}`}
                  >
                    <span>Svenska</span>
                    {lang === 'sv' && '✓'}
                  </button>
                  <button 
                    ...
                    className={`w-full text-left px-3 py-2 text-xs hover:bg-slate-700 flex items-center justify-between touch-manipulation ${lang === 'en' ? 'text-emerald-400 font-bold' : 'text-slate-300'}`}
                  >
                    <span>English</span>
                    {lang === 'en' && '✓'}
                  </button>
                  <button 
                    ...
                    className={`w-full text-left px-3 py-2 text-xs hover:bg-slate-700 flex items-center justify-between touch-manipulation ${lang === 'es' ? 'text-emerald-400 font-bold' : 'text-slate-300'}`}
                  >
                    <span>Español</span>
                    {lang === 'es' && '✓'}
                  </button>
